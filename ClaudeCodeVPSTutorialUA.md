...
## Бонусний крок: Git Relay — щоб Claude сам керував твоїм сервером

Після цього кроку Claude (у claude.ai або Claude Code) зможе **напряму
виконувати команди на твоєму сервері**: перезапускати бота, показувати
логи, робити бекапи, ставити нові бібліотеки — все без того, щоб ти
заходила в Browser terminal.

**Як це працює (простими словами):** на сервері запускається маленький
скрипт-«поштар». Раз на 5 секунд він заглядає в спеціальний файл у твоєму
GitHub-репозиторії — `cmds/pending.json`. Якщо ти (або Claude) поклала туди
команду, він її виконує і повертає результат у `cmds/result.json`. Тобто
Claude передає команди через GitHub, а не через термінал — а GitHub
доступний звідки завгодно.

> **Час налаштування:** 5 хвилин. Один раз. Більше до Browser terminal не
> повертаєшся **взагалі ніколи**.

### Крок 1. Скажи Claude встановити Git Relay

У claude.ai (або Claude Code) у репозиторії `my-bot` напиши:

> Налаштуй мені Git Relay у цьому репозиторії. Це механізм, через який
> ти зможеш виконувати команди на моєму сервері напряму, без термінала.
>
> Контекст:
> - Бот живе в `/root/my-bot/` (або де у тебе бот, скажи Claude правильний шлях)
> - У `.env` є `GITHUB_TOKEN`
> - Автодеплой уже працює: `/root/autodeploy.sh` через systemd-таймер
>   щохвилини робить `git pull`
>
> Створи і закомить у main:
>
> 1. Файл `cmd_runner.py`:
>    - Раз на 5 секунд через GitHub API читає `cmds/pending.json`
>    - Якщо там новий `id` (не дорівнює `last_cmd_id`) — виконує `cmd`
>      як shell-команду через `subprocess.run`
>    - Результат (stdout, stderr, returncode, ts) пушить у
>      `cmds/result.json` через GitHub API
>    - Бере `GITHUB_TOKEN` з оточення або з `.env`
>    - Тільки стандартна бібліотека Python
>    - Таймаут команди 120 секунд
>    - Stdout обрізає до останніх 3000 символів
>
> 2. Файл `cmdrunner.service` (systemd-юніт):
>    - `User=root` (бо команди можуть потребувати `systemctl`)
>    - `WorkingDirectory=/root/my-bot`
>    - `ExecStart=/usr/bin/python3 /root/my-bot/cmd_runner.py`
>    - `Restart=always`, `RestartSec=10`
>    - `EnvironmentFile=-/root/my-bot/.env`
>
> 3. Папку `cmds/` з двома файлами:
>    - `cmds/pending.json`: `{"id":"init","cmd":"echo ready"}`
>    - `cmds/result.json`: `{}`
>
> 4. Допиши в README/CLAUDE.md розділ «Git Relay» — короткий опис
>    механізму та формат JSON.
>
> Закомить одним коммітом «Add Git Relay» і запушь у main.

Claude зробить файли і запушить. За ~1 хвилину автодеплой підтягне їх на
сервер.

### Крок 2. Один раз встановити сервіс — у Browser terminal

Поки Claude пушить — відкрий **Browser terminal** на DigitalOcean
(той самий, що у Кроці 2 раніше). Скопіюй блок і встав:
...
- Копіює юніт-файл туди, де `systemd` його знайде
- Каже `systemd` перечитати юніти
- Включає сервіс і запускає його
- Показує статус

Якщо побачиш `Active: active (running)` — **готово**.

### Крок 3. Перевір що Claude дотягується до сервера

У claude.ai напиши:

> Перевір, що Git Relay працює. Поклади у `cmds/pending.json`:
> `{"id":"hello-test-1","cmd":"echo ready && hostname && uptime"}`,
> запушь у main, зачекай 10 секунд, прочитай `cmds/result.json` і
> покажи мені результат.

Через ~10 секунд Claude покаже тобі вивід твого сервера. Якщо бачиш
`ready`, ім'я сервера і час роботи — **Git Relay працює**.

### Що тепер можна

Просто пиши Claude звичайною мовою:

| Хочу | Пишу |
|---|---|
| Перевірити чи бот живий | *«Перевір статус бота на сервері»* |
| Подивитися логи | *«Покажи мені останні 50 рядків логів бота»* |
| Перезапустити | *«Перезапусти бота»* |
| Поставити бібліотеку | *«Встанови `requests` у venv бота»* |
| Зробити бекап | *«Зроби бекап папки бота в `/tmp`»* |

Все це Claude зробить через Git Relay — без твоєї участі.

⚠️ **Важливо про безпеку:** репозиторій має бути **приватний**. Інакше
будь-хто зможе писати команди, які виконаються на твоєму сервері з
правами root. Якщо репо стало публічним — терміново знеакти токен GitHub
і створи новий.
